...
class ThermostatBase
	...
	save: (cb) =>
		...
		if @id == 0
			Global.query "INSERT INTO thermostats SET ?", columns, (err, result) =>
				sys.puts err if err?
				@id=result.insertId
				cb()
		else
			Global.query "UPDATE thermostats SET ? WHERE id = " + @id, columns, cb
	@load = ( id, cb ) ->
		Global.query "SELECT * FROM thermostats where id = " + id, null, (err, rows) =>
			result = ThermostatBase.loadRow rows[0] if (rows.length>0)
			cb(result);
	@delete = ( id, cb ) ->
		Global.query "DELETE FROM thermostats where id = " + id, null , (err, rows) =>
			cb();
	@loadRow = (row) ->
