...
class SnapshotBase
	...
	save: (cb) =>
		...
		if @id == 0
			Global.getPool().query "INSERT INTO snapshots SET ?", columns, (err, result) =>
				sys.puts err if err?
				@id=result.insertId
				cb()
		else
			Global.getPool().query "UPDATE snapshots SET ? WHERE id = " + @id, columns, cb
	@load = ( id, cb ) ->
		Global.getPool().query "SELECT * FROM snapshots where id = " + id, (err, rows) =>
			result = SnapshotBase.loadRow rows[0] if (rows.length>0)
			cb(result);
	@delete = ( id, cb ) ->
		Global.getPool().query "DELETE FROM snapshots where id = " + id, (err, rows) =>
			cb();
	@loadRow = (row) ->
